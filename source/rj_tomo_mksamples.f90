...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  ! For MPI
  INTEGER ra,rank,nbproc,ierror,rr
  REAL clock
  INTEGER ranseed
  INTEGER date_time(8)
...
INTERFACE
SUBROUTINE updaterays(RayCel_prop,RayCel,Voro_prop,ncell,ncell_prop,cellogic,ind,pn,sn)
		...
		INTEGER			::	switch(nsou*nrec,2)
		REAL(KIND=ii10)		::	p1,p2,point(2)
...
! From now on, the code is run on each processor independently, with ra = processor number.
...
! We now have an INITIAL velocity model made of randomly-located Voronoi cells with random velocity values
...
IF (runnum.NE.1) THEN
	...
	IF (sigdep.EQ.0) THEN
		DO nss=1,nds
			READ(84)sigma(1,nss)
			WRITE(*,*)'Flag for second run of the chain inside.  Sigma update'
		END DO
	...
	IF (birth.EQ.1) THEN ! Add one cell
		...
		IF (a.EQ.1) THEN ! if inside the prior...
			...
			IF(urps.EQ.2) THEN
				...
				ind=1
				!write(*,*)'BIRTH'
				CALL updaterays(RayCel_prop,RayCel,Voro_prop,ncell,ncell_prop,cellogic,ind,rank+1,ount)
...
!***********************************************************************************************
		...
		IF (abs(mean-Voro_pprop(3,ind)).LE.theta) a2=1	! if we are inside the acceptable velocity range
			...
			IF(urps.EQ.2) THEN
				...
				IF (uar.EQ.1) cellogic=.true.
				CALL updaterays(RayCel_pprop,RayCel,Voro_pprop,ncell,ncell_prop,cellogic,ind,rank+1,ount)
...
CALL MPI_FINALIZE(ierror)! Terminate the parallelization
...
IF (rank.EQ.0) WRITE(*,*)
IF (rank.EQ.0) WRITE(*,*)'------------------------------------------------------------------------'
IF (rank.EQ.0) WRITE(*,*)'Time taken by the code was',t2-t1,'seconds'
	...
	SUBROUTINE updaterays(RayCel_prop,RayCel,Voro_prop,ncell,ncell_prop,cellogic,ind,pn,sn)
		...
		IF(nru.GT.0) THEN
			...
			CALL modrays(nsou,sou(:,1),sou(:,2),&
				nrec,rec(:,1),rec(:,2),&
				switch,-1,&
				nvtr,nvpr,latmax,longmin,latr,longr,RAYUP,&
				gdt,gdp,&
				sgref,&
				dl,erg,&
				er,&
