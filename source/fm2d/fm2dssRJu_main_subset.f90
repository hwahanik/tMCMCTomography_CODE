!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  Version with sources subset input for mpi parallelization
!  Version written by Helmut Wahanik Research Scientist
!  Schlumberger Research Oct. 2014
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! MAIN PROGRAM
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! TYPE: PROGRAM   
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ! but restricted to the set of sources calculated inside of this worker

    ! nsrc,scx,scz					! sources subset (degrees)
    ! nrc,rcx,rcz					! receivers (degrees)
    ! srs, wrgf  					! source-receiver associations (as in first column of otimes.dat), 
							! restricted to the sources subset 
							! wrgf = write geometries to file? 
    ...
    ! proc 						! processor
    ! nru_subset					! number of rays to update

SUBROUTINE modrays_subset(nsrcin,scxin,sczin,&
		nrcin,rcxin,rczin,&
		srsin,wrgfin,&
...
ALLOCATE(npoints_subset(nru_subset,2))
...
!write(*,*)'Call GRIDDER'
 ...
 CALL gridder_subset(nvxin,nvzin,goxdin,gozdin,dvxdin,dvzdin,velvin)
...
!
   ...
   IF(wrgf.eq.i.OR.wrgf.LT.0)THEN
	!write(*,*)'calling RPATHS to model',nru_subset,'raypaths'
      CALL rpaths_subset(wrgf,i,x,z,proc)
      
      crazyray_subset=crazyray_subset+crazyrp
	...
	IF(ALLOCATED(raycoords_subset)) THEN
      		!write(*,*)'raycoords_subset allocated'
		nold=size(raycoords_subset,1)
        	ALLOCATE(tmpcoords(nold,size(raycoords_subset,2)))
      		tmpcoords=raycoords_subset
		DEALLOCATE(raycoords_subset)
		ALLOCATE(raycoords_subset(nold+newcoords,2))
		raycoords_subset(1:nold,:)=tmpcoords(:,:)
		raycoords_subset(nold+1:nold+newcoords,:)=raypts(:,:)
		DEALLOCATE(tmpcoords)
		DEALLOCATE(raypts)
      	ELSE
      		!write(*,*)'raycoords_subset not allocated' 
      		ALLOCATE(raycoords_subset(newcoords,size(raypts,2)))
		raycoords_subset=raypts
		DEALLOCATE(raypts)
      	END IF
	
      	npoints_subset(nploci:nplocf,:)=npts
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE gridder_subset(nvxin,nvzin,goxdin,gozdin,dvxdin,dvzdin,velvin)
USE globalp
IMPLICIT NONE
...
!IF(checkstat > 0)THEN
!   WRITE(6,*)'Error with ALLOCATE: SUBROUTINE gridder_subset: REAL velv'
!ENDIF
!DO i=0,nvz+1
!   DO j=0,nvx+1
!      READ(10,*)velv(i,j)
...
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with ALLOCATE: SUBROUTINE gridder_subset: REAL veln'
ENDIF
!
...
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with ALLOCATE: Subroutine gridder_subset: REAL ui'
ENDIF
DO i=1,gdx+1
   u=gdx
   u=(i-1)/u
...
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with ALLOCATE: Subroutine gridder_subset: REAL vi'
ENDIF
DO i=1,gdz+1
   u=gdz
   u=(i-1)/u
...
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with DEALLOCATE: SUBROUTINE gridder_subset: REAL ui,vi'
ENDIF
END SUBROUTINE gridder_subset
...
DO i=1,nvz-1
   ...
   ENDDO
ENDDO
END SUBROUTINE bsplrefine_subset
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE srtimes_subset(scx,scz,csid)
USE globalp
IMPLICIT NONE
...
!
      ...
      CALL bilinear_subset(vss,drx,drz,vels)
      DO k=1,2
         DO l=1,2
      ...
      CALL bilinear_subset(vss,drx,drz,velr)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE rpaths_subset(wrgf,csid,scx,scz,procn)
USE globalp
IMPLICIT NONE
!INTEGER, PARAMETER :: i5=SELECTED_REAL_KIND(5,10)
...
IF(ALLOCATED(npts)) THEN
	...
	IF(checkstat > 0)THEN
   		WRITE(*,*)'Error with DEALLOCATE: SUBROUTINE rpaths_subset: npts'
	ENDIF
END IF
IF(ALLOCATED(raypts)) THEN
	...
	IF(checkstat > 0)THEN
   		WRITE(*,*)'Error with DEALLOCATE: SUBROUTINE rpaths_subset: raypts'
	ENDIF
END IF
...
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with ALLOCATE: SUBROUTINE rpaths_subset: REAL rgx'
ENDIF
ALLOCATE(rgz(maxrp+1), STAT=checkstat)
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with ALLOCATE: SUBROUTINE rpaths_subset: REAL rgz'
ENDIF
!
...
!   IF(checkstat > 0)THEN
!      WRITE(6,*)'Error with ALLOCATE: SUBROUTINE rpaths_subset: REAL fdm'
!   ENDIF
!ENDIF
!
...
!
	...
	ELSE IF(srrecip.EQ.1) THEN ! Apply source-receiver reciprocity
			   ...
			   IF(checkstat > 0)THEN
			      WRITE(*,*)'Error with ALLOCATE: SUBROUTINE rpaths_subset: REAL rgxnew'
			   ENDIF
			   ALLOCATE(rgznew(maxrp+1), STAT=checkstat)
			   IF(checkstat > 0)THEN
			      WRITE(*,*)'Error with ALLOCATE: SUBROUTINE rpaths_subset: REAL rgznew'
			   ENDIF
			...
			!
				...
				IF(checkstat > 0)THEN
				   WRITE(*,*)'Error with DEALLOCATE: SUBROUTINE rpaths_subset: rgxnew,rgznew'
				ENDIF
				sw=1
				EXIT
...
!   IF(checkstat > 0)THEN
!      WRITE(6,*)'Error with DEALLOCATE: SUBROUTINE rpaths_subset: fdm'
!   ENDIF
!ENDIF
1866	CONTINUE
IF(ALLOCATED(rgx)) DEALLOCATE(rgx,rgz, STAT=checkstat)
IF(checkstat > 0)THEN
   WRITE(*,*)'Error with DEALLOCATE: SUBROUTINE rpaths_subset: rgx,rgz'
ENDIF
END SUBROUTINE rpaths_subset
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE bilinear_subset(nv,dsx,dsz,biv)
USE globalp
IMPLICIT NONE
INTEGER :: i,j
...
DO i=1,2
   ...
   ENDDO
ENDDO
END SUBROUTINE bilinear_subset
