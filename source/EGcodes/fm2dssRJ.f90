...
!!!!! Code modified by Erica Galetti (EG) in order to produce a binary raypath
!!!!! file (here called wrays) compatible with the RJ_MCMC tomography code.
!!!!! [In the original version of fm2dss.f90, the raypath.out 
!!!!! file was binary and incompatible with the RJ_MCMC code.]
!!!!! In addition, the file of Frechet derivatives is saved as both 
!!!!! binary and text (only binary in orignal fm2dss code).
...
!
IF(cfd.EQ.1)THEN
   OPEN(UNIT=50,FILE=trim(frechet) // '.bin',FORM='unformatted',STATUS='unknown')
   OPEN(UNIT=51,FILE=trim(frechet) // '.txt',FORM='formatted',STATUS='unknown')
ENDIF
!
...
!
      ...
      DO j=0,nvz+1
         DO k=0,nvx+1
            ...
            IF(ABS(fdm(j,k)).GE.ftol)WRITE(51,*)isum,fdm(j,k)
         ENDDO
      ENDDO
   ENDIF
ENDDO
