...
!!											!!
!! Hybrid embarrasingly-parallel and mpi code: rays-routines MPI parallelization.	!!
...
rank = 0     ! used for testing seeral single jobs submission
...
IF (taskid.GT.0) THEN 
    ...
    DO WHILE (run_worker.EQ.0)	! <<<--------------------LOOP OVER MPI PROCESSORS STARTS HERE !!! ----------------------------
	    ...
	    !OK All checked!
    END DO	   
500 WRITE(*,*) 'Finished loop at worker'
END IF 
