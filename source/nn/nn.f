...
c	      Following 3 lines modified by EG to optimize the subroutine for rj-McMC tomography
              ptsok=0
              call delaun (points,np,neighbour,vertices,nt,nt_max,
     &                     vis_tlist,vis_elist,add_tlist,eps,nv_max,
     &                     0,ldummy,0,0,0,ptsok)
	      if (ptsok.eq.1) then
		WRITE(*,*)'All',np,'points are in a line!'
		STOP
	      end if
...
c               num_nn                  number of natural neighbours of
c                                       input point
c               nn_ind(num_nn)          indices of natural neighbour nodes 
c               cnn(num_nn)             natural neighbour co-ordinates
c                                       of node with respect to its neighbours.
c               cnn_d(2,num_nn)         first derivatives of natural neighbour
c                                       co-ordinates
c
c	Comments:
...
c
c		 Arrays cnn,cnn_d,cnn_dd,nn_ind are only calculated
c		 with int_method = 4 (closed form).
c
...
c
	Subroutine nn2D
     ...
     &   work_d5,work_d6,work_d7,lt_work,ln_work,out,
     &   num_nn,nn_ind,cnn,cnn_d,cnn_dd,f,df,ddf,v)

	double precision		points(2,*)
	double precision		centres(3,*)
	double precision		xd(2),x,y
	double precision		data(*)
        double precision          cnn(nnpn_max)
        double precision          cnn_d(2,nnpn_max)
        double precision          cnn_dd(3,nnpn_max)
	double precision		f,df(2),ddf(3),v
	integer		vertices(3,*)
	integer		neighbour(3,*)
        integer		hulltriangles(*)
	integer		int_method
	integer		nn_ind(nnpn_max)
	logical		out
	logical		nnext
	logical		nnwrite

        double precision          work_d1(2,nnpn_max)
        double precision          work_d2(2,nnpn_max)
        double precision          work_d3(2,nnpn_max)
        double precision          work_d4(2,nnpn_max)
        double precision          work_d5(2,nnpn_max)
        double precision          work_d6(2,nnpn_max)
        double precision          work_d7(2,nnpn_max)
...
c
     ...
     &              work_i2,work_r2,work_i3,lt_work,ln_work,
     &              num_nn,nn_ind,cnn,f,v)
...
c
     ...
     &              lt_worK,ln_work,num_nn,nn_ind,cnn,cnn_d,f,df,v)
...
c
     ...
     &              lt_work,ln_work,num_nn,nn_ind,cnn,cnn_d,cnn_dd,
     &              f,df,ddf,v)
...
c
     ...
     &  (x,y,points,vertices,loc,data,f,df,v)
        ...
        double precision          x1,y1,x2,y2,x3,y3,A,a1,a2,a3
        double precision          dx12,dx13,dy12,dy13,df12,df13
        double precision          data(*)
        double precision          f,df(2),v
        integer         vertices(3,*)
...
c------------------------------------------------------------------------
c
c	build_nt - Builds list of triangles attached to each node in 2-D.
c
c	Input:	
c	        vertices(3,nt)		array of triangle vertices	
c	        nt			number of triangles
...
c	Output:
c		neighbour(3,nt)		array of neighbouring triangles
c		nnn(np+1)               pointer array for ntlist (and nnlist)
c		ntlist			array of triangles attached to each node
c
c	Comments:
c		 This routine builds the list of triangles attached to each 
c		 node. It is not used in the nn routines but may be used 
c		 to find the vertices of any Voronoi cell. For example to
c		 to plot it. 
c
c		 The routine nnn is recalculated here but is exactly the same
c		 as produced by routine build_nv which builds both the 
c		 neighbour array and the list of natural neighbour nodes 
c		 attached to each node.
c
c		 No calls to other routines.
c
...
c------------------------------------------------------------------------
c
        subroutine build_nt(vertices,nt,np,ntlist,nnn,nmax)
...
c                                       reset pointers for packed arrays
        do i = np+1,2,-1
           nnn(i) = nnn(i-1) + 1
        end do
        nnn(1) = 1
...
c
	...
	integer		p1,p2
        logical		out
	double precision		x,y,del1,del2
        integer		c1(3)
        data		c1/2,3,1/
c
	out = .false.
...
c------------------------------------------------------------------------
c
c	find_node2D - locates the nearest node to point x,y using natural 
c		      neighbour lists calculated by nn2d_setup routine.
c
c	Input:
...
c------------------------------------------------------------------------
c
        Subroutine find_node2D (xd,nodein,points,nnn,nnlist,walk)
        ...
        double precision          dist,a,b,dmin
        integer         walk
...
c                                       list to find nearest node
        ...
        a = points(1,noden)-xd(1)
        b = points(2,noden)-xd(2)
        dmin = a*a + b*b

c       write(*,*)'inside findnode starting node=',noden,' d=',dmin

 20     continue
c       do i=nnn(noden),nnn(noden+1)-1
        do i=nnn(noden),nnn(noden+1)-1
           ...
           if(n.ne.0)then
              ...
              if(dist.lt.dmin)then
                 noden=n
                 dmin = dist
c                write(*,*)'findnode : newnode=',noden,' d ',dmin
                 walk = walk + 1
                 go to 20
              end if
           end if
        end do
c                                       found nearest node

        nodein = noden
c       write(*,*)'findnode : found node=',nodein,' d ',dmin
...
c
	double precision		points(2,*)
	double precision		centres(3,*)
	double precision		x1,x2,x3,y1,y2,y3,x,y
	double precision		dx2m1,dx2p1,dy2m1,dy2p1
	double precision		dx3m1,dx3p1,dy3m1,dy3p1
...
c
	...
	double precision		centres(3,*)
	double precision		data(*)
	double precision		x,y,dist,dx,dy
	double precision		f,df(2),v,dvx,dvy
	integer		vertices(3,*)
	integer		neighbour(3,*)
	integer		tri,pos
	logical		nnwrite
...
c
	...
	double precision		points(2,*)
	double precision		centres(3,*)
	double precision		x,y,xc,yc,dist
	double precision		data(*)
	double precision		f,df(2),v,dvx,dvy
	integer		vertices(3,*)
	integer		neighbour(3,*)
...
c
	...
	double precision		points(2,*)
	double precision		centres(3,*)
	double precision		x1,x2,f,df(2),v
	double precision		dvx,dvy,a,b,dxw,dyw,det
	double precision		p(2,3),c(2,3),f1(2,3),f2(2,3),cx,cy
	double precision		dcx(2,3),dcy(2,3)
	double precision		data(*)
	integer		vertices(3,*)
	integer		tri
	logical		nnwrite
...
c
	...
	double precision		x,y,dist,dx,dy
	double precision		f,df(2),v
	double precision		xo(2)
        double precision  	xs(2)
        double precision          p(2,nnpn_max)
	integer		vertices(3,*)
	integer		neighbour(3,*)
...
c
	Subroutine nn2Df
     ...
     &   pverts,vverts,tstore,vpairs,angles,index,ltri,lnode,
     &   num_nn,nn_ind,cnn,f,v)

	double precision		points(2,*)
	double precision		centres(3,*)
	double precision		data(*)
	double precision		x(2),dist,dx,dy
	double precision		f,v
        double precision          pverts(2,nnpn_max)
        double precision          vverts(2,nnpn_max)
        double precision          cnn(nnpn_max)
	integer		vertices(3,*)
	integer		neighbour(3,*)
...
c               cnn(num_nn)             natural neighbour co-ordinates
c                                       of node with respect to its neighbours.
c               cnn_d(3,num_nn)         first derivatives of natural neighbour
c                                       co-ordinates
c
c	Comments:
c		 On input point x must be in triangle loc. 
...
c
	Subroutine nn2Dfd
     ...
     &   ltri,lnode,num_nn,nn_ind,cnn,cnn_d,f,df,v)

	double precision		points(2,*)
	double precision		centres(3,*)
	double precision		data(*)
	double precision		x(2),dist,dx,dy
	double precision		f,v,df(2),deriv(2),dvx,dvy
        double precision          pverts(2,nnpn_max)
        double precision          vverts(2,nnpn_max)
        double precision          dx_verts(2,nnpn_max)
        double precision          dy_verts(2,nnpn_max)
        double precision          dpverts(4,2)
        double precision          cnn(nnpn_max)
        double precision          cnn_d(2,nnpn_max)
        real            angles(nnpn_max)
	integer		vertices(3,*)
...
c
              ...
              dvx = dvx + deriv(1)
              dvy = dvy + deriv(2)
              cnn_d(1,num_nn) = deriv(1)
              cnn_d(2,num_nn) = deriv(2)
 ...
 120    continue
 	if(v.ne.0.0)then
           ...
           do i=1,num_nn
              cnn(i) = cnn(i)/v
              cnn_d(1,i) = (cnn_d(1,i) - cnn(i)*dvx)/v
              cnn_d(2,i) = (cnn_d(2,i) - cnn(i)*dvy)/v
           end do
        end if
        v = v*0.5
c
...
c               cnn_d(2,num_nn)         first derivatives of natural neighbour
c                                       co-ordinates
c               cnn_dd(3,num_nn)        second derivatives of natural neighbour
c                                       co-ordinates with respect node position
c
c	Comments:
c		 On input point x must be in triangle loc. 
c		 Note: The input triangle contains x and so it's 
...
c
	Subroutine nn2Dfdd
     &  (x,points,vertices,neighbour,centres,loc,data,nnpn_max,
     &   pverts,vverts,tstore,vpairs,angles,index,dx_verts,dy_verts,
     &   dxx_verts,dyy_verts,dxy_verts,ltri,lnode,
     &   num_nn,nn_ind,cnn,cnn_d,cnn_dd,f,df,ddf,v)
	...
	double precision	data(*)
	double precision	x(2),dist,dx,dy
	double precision	f,v,df(2),ddf(3)
        double precision	deriv(2),dderiv(3),dvx,dvy,dvxx,dvyy,dvxy
        double precision          pverts(2,nnpn_max)
        double precision          dpverts(4,2)
        double precision          ddpverts(6,2)
        double precision          vverts(2,nnpn_max)
        double precision          dx_verts(2,nnpn_max)
        double precision          dy_verts(2,nnpn_max)
        double precision          dxx_verts(2,nnpn_max)
        double precision          dyy_verts(2,nnpn_max)
        double precision          dxy_verts(2,nnpn_max)
        double precision          cnn(nnpn_max)
        double precision          cnn_d(2,nnpn_max)
        double precision          cnn_dd(3,nnpn_max)
	integer		vertices(3,*)
...
c
              ...
              cnn_d(1,num_nn) = deriv(1)
              cnn_d(2,num_nn) = deriv(2)
              cnn_dd(1,num_nn) = dderiv(1)
              cnn_dd(2,num_nn) = dderiv(2)
              cnn_dd(3,num_nn) = dderiv(3)
 ...
 120    continue
 	if(v.ne.0.0)then
           ...
           do i=1,num_nn
              ...
              cnn_d(1,i) = (cnn_d(1,i) - cnn(i)*dvx)/v
              cnn_d(2,i) = (cnn_d(2,i) - cnn(i)*dvy)/v
              cnn_dd(1,i) = (cnn_dd(1,i) -2.*dvx*cnn_d(1,i)
     &                       - dvxx*cnn(i))/v
              cnn_dd(2,i) = (cnn_dd(2,i) -2.*dvy*cnn_d(2,i)
     &                       - dvyy*cnn(i))/v
              cnn_dd(3,i) = (cnn_dd(3,i) - dvy*cnn_d(1,i)
     &                       -dvx*cnn_d(2,i) -dvxy*cnn(i))/v
           end do
        end if
        v = v*0.5
c
...
c
      ...
      double precision	p(nmax,mmax)
      double precision    xo(nmax)
      real  	a(mmax,nmax)
      real	b(mmax)
...
c
      ...
      INTEGER n
c     REAL ra(n)
      double precision ra(2,n)
      double precision da(n)
      INTEGER i,ir,j,l,ka
c     REAL rra
      double precision rra(2)
...
c
	double precision		pa(2),pb(2),pc(2)
...
c
	...
	double precision		dx3m1,dx3p1,dy3m1,dy3p1
	double precision		denom
	double precision		vx(2),vy(2),vxx(2),vyy(2),vxy(2)
c
c						Find centre of circum-circle
	   x1 = pa(1)
	   x2 = pb(1)
...
c
      ...
      INTEGER n
      REAL ra(n)
      double precision rb(2,n)
      INTEGER i,ir,j,l
      REAL rra
      double precision rrb1,rrb2
      if (n.lt.2) return
      l=n/2+1
      ir=n
10    continue
...
c
      ...
      double precision	p(2,*)
      double precision	dp(4,2)
      double precision	ddp(6,2)
      double precision	df(2),ddf(3)
...
c       t2 = tarray(2)
        t1 = 0
        t2 = 0
        call na_walltime(timedouble)
        cputime = sngl(timedouble)
